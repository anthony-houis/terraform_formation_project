...
module "attached_iam_policy_with_cluster_role" {
  ...
  source      = "../modules/attach_policy_with_iam_role"
  role_name   = var.cluster_role["role_name"]
...
resource "aws_eks_cluster" "eks_cluster" {
  ...
  vpc_config {
    subnet_ids = [aws_subnet.formation_public_subnet[0].id, aws_subnet.formation_public_subnet[1].id, aws_subnet.formation_public_subnet[2].id]
  }
  depends_on = [
    aws_ec2_tag.tag_subnets,
    module.attached_policy_with_cluster_role
  ]
  tags = {
    "Name" = var.Name
  }
...
resource "aws_ec2_tag" "tag_subnets" {
  for_each    = { for idx, subnet in aws_subnet.formation_public_subnet : idx => subnet }
  resource_id = element(aws_subnet.formation_public_subnet, each.key).id
